...
* マクロ定義
*******************************************************************************/
#define BACKGROUNDCLEAR_USE_TRIANGLE_NUM		( 64 )	//使用三角形数
#define USE_VERTEX_NUM	( BACKGROUNDCLEAR_USE_TRIANGLE_NUM + 2 )		//使用頂点数
...
/*******************************************************************************
* 関数名	:HRESULT InitAfBackGroundClear( void )
*		背景初期化関数です。
* 引数		:void 引数なし
* 戻り値	:HRESULT 関数の実行結果
* 説明		:変数等の初期化を実行する起動毎に一度のみ動作する関数です。
*******************************************************************************/
HRESULT InitAfBackGroundClear( void )
{
	HRESULT result = InitDirectAfBackGroundClear();
	InitValueAfBackGroundClear();
	return result;
}

/*******************************************************************************
* 関数名	:HRESULT InitDirectAfBackGroundClear( void )
*		背景初期化関数です。
* 引数		:void 引数なし
* 戻り値	:HRESULT 関数の実行結果
* 説明		:変数等の初期化を実行する起動毎に一度のみ動作する関数です。
*******************************************************************************/
HRESULT InitDirectAfBackGroundClear( void )
{
	...
	/*テクスチャの読み込み*/
	D3DXCreateTextureFromFile( pD3DDevice , "data/TEXTURE/AfBackgroundClear.png" , &g_pTextureBack );
...
/*******************************************************************************
* 関数名	:HRESULT InitValueAfBackGroundClear( void )
*		背景初期化関数です。
* 引数		:void 引数なし
* 戻り値	:HRESULT 関数の実行結果
* 説明		:変数等の初期化を実行する起動毎に一度のみ動作する関数です。
*******************************************************************************/
void InitValueAfBackGroundClear( void )
{
	writeVector = 0;
...
/*******************************************************************************
* 関数名	:void UninitAfBackGroundClear( void )
*		背景終了処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:終了時に実行する関数を内包する関数で終了時に一度のみ動作。
*******************************************************************************/
void UninitAfBackGroundClear( void )
{
...
/*******************************************************************************
* 関数名	:void UpdateAfBackGroundClear( void )
*		背景更新処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:処理周回毎の処理内容を記述します。実質的なメイン関数です。
*******************************************************************************/
void UpdateAfBackGroundClear( void )
{
	VERTEX_2D *pVtx;	/*頂点バッファへのポインタ*/
	...
	writePosX = SCREEN_WIDTH / 2;
	writePosY = SCREEN_HEIGHT / 1.5 * 1;
	if( getViewStatus()->centerPosY <= 300 )
	{
		fade = ( 255 + -255 * ( getViewStatus()->centerPosY / 300 ) ) * cos( getViewStatus()->centerVector );
	}
	...
	for( int cnt = 1; cnt < USE_VERTEX_NUM; cnt++ )
	{
		...
		//テクスチャ座標設定1
		pVtx[cnt].tex = D3DXVECTOR2( 0.5f + 1.0f * -cos( D3DX_PI * frameVertex ) ,  0.5f + 1.0f * -sin( D3DX_PI * frameVertex ) );
		//メモ：pVtx[0].texでやると気持ち悪くも面白い動きをする。

		frameVertex += 2.0f / BACKGROUNDCLEAR_USE_TRIANGLE_NUM;
...
/*******************************************************************************
* 関数名	:void DrawAfBackGroundClear( void )
*		背景描画処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:画面出力に関わる処理を内包します。
*******************************************************************************/
void DrawAfBackGroundClear( void )
{
	LPDIRECT3DDEVICE9 pD3DDevice = GetDevice();	/*デバイスの取得*/
	...
	for( int cnt = 0; cnt < USE_VERTEX_NUM; cnt++ )
	{
		...
		/*背景の描画*/
		pD3DDevice->DrawPrimitive( D3DPT_TRIANGLEFAN , 0 , BACKGROUNDCLEAR_USE_TRIANGLE_NUM );
	}
