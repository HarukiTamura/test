...
/*******************************************************************************
* 関数名	:HRESULT InitBackPlusAlpha2( void )
*		背景初期化関数です。
* 引数		:void 引数なし
* 戻り値	:HRESULT 関数の実行結果
* 説明		:変数等の初期化を実行する起動毎に一度のみ動作する関数です。
*******************************************************************************/
HRESULT InitBackPlusAlpha2( void )
{
	...
	/*頂点座標設定*/
	pVtxBack2[0].pos = D3DXVECTOR3( 0.0f ,  BAR_POSY , 0.0f );
	pVtxBack2[1].pos = D3DXVECTOR3( WINDOW_WIDTH ,  BAR_POSY , 0.0f );
	pVtxBack2[2].pos = D3DXVECTOR3( 0.0f ,  BAR_POSY + BAR_HEIGHT , 0.0f );
	pVtxBack2[3].pos = D3DXVECTOR3( WINDOW_WIDTH ,  BAR_POSY + BAR_HEIGHT , 0.0f );

	pVtxBack2[4].pos = D3DXVECTOR3( 0.0f ,  BAR2_POSY , 0.0f );
	pVtxBack2[5].pos = D3DXVECTOR3( WINDOW_WIDTH ,  BAR2_POSY , 0.0f );
	pVtxBack2[6].pos = D3DXVECTOR3( 0.0f ,  BAR2_POSY + BAR_HEIGHT , 0.0f );
	pVtxBack2[7].pos = D3DXVECTOR3( WINDOW_WIDTH ,  BAR2_POSY + BAR_HEIGHT , 0.0f );
	...
	/*頂点カラー設定*/
	pVtxBack2[0].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );
	pVtxBack2[1].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );
	pVtxBack2[2].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );
	pVtxBack2[3].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );

	pVtxBack2[4].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );
	pVtxBack2[5].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );
	pVtxBack2[6].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );
	pVtxBack2[7].col = D3DCOLOR_RGBA( 255 , 255 , 255 , 155 );

	/*テクスチャ座標設定*/
	pVtxBack2[0].tex = D3DXVECTOR2( 0.0 , 0.5 );
	pVtxBack2[1].tex = D3DXVECTOR2( 0.5 , 0.5 );
...
/*******************************************************************************
* 関数名	:void UninitBackPlusAlpha2( void )
*		背景終了処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:終了時に実行する関数を内包する関数で終了時に一度のみ動作。
*******************************************************************************/
void UninitBackPlusAlpha2( void )
{
...
/*******************************************************************************
* 関数名	:void UpdateBackPlusAlpha2( void )
*		背景更新処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:処理周回毎の処理内容を記述します。実質的なメイン関数です。
*******************************************************************************/
void UpdateBackPlusAlpha2( void )
{
	VERTEX_2D *pVtxBack2;	/*頂点バッファへのポインタ*/

	texPos = ( texPos >= 1.0 ? 0 : texPos + 0.0005 );
	
	/*ポインタのロック*/
	g_pVtxBuffPolygon2->Lock( 0 , 0 , ( void** ) &pVtxBack2 , 0 );
	
	/*テクスチャ座標設定*/
	pVtxBack2[0].tex = D3DXVECTOR2( 0.0 - texPos , 0.5 );
	pVtxBack2[1].tex = D3DXVECTOR2( 2.0 - texPos , 0.5 );
	pVtxBack2[2].tex = D3DXVECTOR2( 0.0 - texPos , 1.0 );
	pVtxBack2[3].tex = D3DXVECTOR2( 2.0 - texPos , 1.0 );
...
/*******************************************************************************
* 関数名	:void DrawBackPlusAlpha2( void )
*		背景描画処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:画面出力に関わる処理を内包します。
*******************************************************************************/
void DrawBackPlusAlpha2( void )
{
	LPDIRECT3DDEVICE9 pD3DDevice = GetDevice();	/*デバイスの取得*/
