...
* マクロ定義
*******************************************************************************/
#define PAUSE_GRAPHIC_VERTEX_MAX	( 4 )	/*最高頂点数*/

#define PAUSE_GRAPHIC_OUTPUT_POS_X	( SCREEN_WIDTH * 0.5 )		/*表示X座標*/
#define PAUSE_GRAPHIC_OUTPUT_POS_Y	( SCREEN_HEIGHT * 0.5 )		/*表示Y座標*/

#define PAUSE_GRAPHIC_ANIM_MAX		( 1 )	/*アニメーションコマ数*/
#define PAUSE_GRAPHIC_ANIM_SPAN		( 0 )	/*アニメーション間隔*/
#define PAUSE_GRAPHIC_ANIM_DIV_X	( 1 )	/*幅分割数*/
#define PAUSE_GRAPHIC_ANIM_DIV_Y	( 1 )	/*高さ分割数*/

#define PAUSE_GRAPHIC_OUTPUT_SIZE	( 1 ) /*初期表示倍率*/ 
...
* グローバル変数
*******************************************************************************/
static LPDIRECT3DVERTEXBUFFER9 g_pVtxBuffPauseGraphic = NULL;	/*頂点バッファ*/
static LPDIRECT3DTEXTURE9 g_pTexturePauseGraphic;	/*ポーズグラフィックテクスチャのポインタ*/
...
/*******************************************************************************
* 関数名	:HRESULT InitPauseGraphic( void )
*		ポーズグラフィック初期化関数です。
* 引数		:void 引数なし
* 戻り値	:HRESULT 関数の実行結果
* 説明		:変数等の初期化を実行する起動毎に一度のみ動作する関数です。
*******************************************************************************/
HRESULT InitPauseGraphic( void )
{
	g_nCounterAnim = 0;
	g_nPatternAnim = 0;
	...
	/*頂点バッファの生成*/
	if( FAILED( pD3DDevice->CreateVertexBuffer( sizeof( VERTEX_2D ) * PAUSE_GRAPHIC_VERTEX_MAX , D3DUSAGE_WRITEONLY , FVF_VERTEX_2D , D3DPOOL_MANAGED , &g_pVtxBuffPauseGraphic , NULL ) ) )
	{
	...
	/*テクスチャの読み込み*/
	D3DXCreateTextureFromFile( pD3DDevice , "data/TEXTURE/pauseGraphic.png" , &g_pTexturePauseGraphic );
	D3DXGetImageInfoFromFile( "data/TEXTURE/pauseGraphic.png" , &info );

	/*ポインタのロック*/
	g_pVtxBuffPauseGraphic->Lock( 0 , 0 , ( void** ) &pVtx , 0 );
	
	
	outputSizeX = ( int )( ( info.Width / PAUSE_GRAPHIC_ANIM_DIV_X ) * PAUSE_GRAPHIC_OUTPUT_SIZE );
	outputSizeY = ( int )( ( info.Height / PAUSE_GRAPHIC_ANIM_DIV_Y ) * PAUSE_GRAPHIC_OUTPUT_SIZE );

	g_posPolygon.x = PAUSE_GRAPHIC_OUTPUT_POS_X;
	g_posPolygon.y = PAUSE_GRAPHIC_OUTPUT_POS_Y;
	g_posPolygon.z = 0.0f;
...
/*******************************************************************************
* 関数名	:void UninitPauseGraphic( void )
*		ポーズグラフィック終了処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:終了時に実行する関数を内包する関数で終了時に一度のみ動作。
*******************************************************************************/
void UninitPauseGraphic( void )
{
	if( g_pVtxBuffPauseGraphic != NULL )
	{
		g_pVtxBuffPauseGraphic->Release();
		g_pVtxBuffPauseGraphic = NULL;
	}
	if( g_pTexturePauseGraphic != NULL )
	{
		g_pTexturePauseGraphic->Release();
		g_pTexturePauseGraphic = NULL;
	}
}

/*******************************************************************************
* 関数名	:void UpdatePauseGraphic( void )
*		ポーズグラフィック更新処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:処理周回毎の処理内容を記述します。実質的なメイン関数です。
*******************************************************************************/
void UpdatePauseGraphic( void )
{
	VERTEX_2D *pVtx;	/*頂点バッファへのポインタ*/
	float texWidth , texHeight , nowPatternX , nowPatternY;
	
	if( ++g_nCounterAnim == PAUSE_GRAPHIC_ANIM_SPAN )
	{
		g_nCounterAnim = 0;
		g_nPatternAnim = ( ++g_nPatternAnim == PAUSE_GRAPHIC_ANIM_MAX ? 0 : g_nPatternAnim ); 
	}
	
	texWidth = 1.0f / PAUSE_GRAPHIC_ANIM_DIV_X;
	texHeight = 1.0f / PAUSE_GRAPHIC_ANIM_DIV_Y;
	nowPatternX = ( float )( g_nPatternAnim % PAUSE_GRAPHIC_ANIM_DIV_X );
	nowPatternY = ( float )( ( int )( g_nPatternAnim / PAUSE_GRAPHIC_ANIM_DIV_X ) );
	/*ポインタのロック*/ 
	g_pVtxBuffPauseGraphic->Lock( 0 , 0 , ( void** ) &pVtx , 0 );
...
/*******************************************************************************
* 関数名	:void DrawPauseGraphic( void )
*		ポーズグラフィック描画処理関数です。
* 引数		:void 引数なし
* 戻り値	:void 戻り値なし
* 説明		:画面出力に関わる処理を内包します。
*******************************************************************************/
void DrawPauseGraphic( void )
{
	LPDIRECT3DDEVICE9 pD3DDevice = GetDevice();	/*デバイスの取得*/
	...
	/*テクスチャの設定*/
	pD3DDevice->SetTexture( 0 , g_pTexturePauseGraphic );
	
	/*頂点バッファをストリームにバインド*/
	pD3DDevice->SetStreamSource( 0 , g_pVtxBuffPauseGraphic , 0 , sizeof( VERTEX_2D ) );
